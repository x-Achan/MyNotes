...
**给定一个索引，返回对应的堆叠帧张量和，和对应的标签信息（frames，targets）
...
class UAVDTDetectionDataset(Dataset):  
    ...
    '''  
        - ann_path是一个json的路径
        - 给定一个json文件的路径，将json的内容解析出来，
        - 保存到一个字典中（图片数据路径、边界框list、标签list、追踪id、原始图片高和宽）  
    ''' 
    ...
    '''  
        ...
        3. 将所有帧堆叠起来保存在 frames 数组（PIL）中
        4. 应用变换，将其转为 torch.Tensor [C=3, T=8, H=224, W=224]   
    '''    
    def _load_and_transform_frames(self, img_path: str) -> torch.Tensor:  
        ...
        if img_path is None or not os.path.exists(img_path):  
            ...
            return frames  
        try:  
            # 尝试用 PIL 打开图片并转换为 RGB            img = Image.open(img_path).convert('RGB')  
        ...
        # 提取帧编号，从 img000001.jpg中提取 000001        import re  
        match = re.search(r'(\d+)', frame_name) #r'(\d+)'匹配连续数字  
        if match:  
            frame_idx = int(match.group(1)) # 帧编号 1（转为整数）  
            prefix = frame_name[:match.start()] # 数字前的部分 img            ext = frame_name[match.end():] #数字后的部分，文件扩展名 .jpg        else:  
            # 用相同的参考帧复制 clip_len次  
    ...
    def _apply_transforms(self, frames: list) -> torch.Tensor:  
        ...
        for frame in frames:  
            ...
            frame_tensor = F.to_dtype(frame_tensor, dtype=torch.float32, scale=True) #  转为 float32 并归一化到 [0,1]            frame_tensors.append(frame_tensor) # 将处理后的张量添加到列表  
        ...
        # 转换为 [C, T, H, W]        clip = clip.permute(1, 0, 2, 3)  
    ...
    def _normalize_boxes(self, boxes: list, orig_size: list) -> torch.Tensor:  
        ...
        # 从角点坐标转换为中心坐标格式 Convert (x1, y1, x2, y2) to (cx, cy, w, h)        cx = (boxes_tensor[:, 0] + boxes_tensor[:, 2]) / 2  
        cy = (boxes_tensor[:, 1] + boxes_tensor[:, 3]) / 2  
        w = boxes_tensor[:, 2] - boxes_tensor[:, 0]  
        h = boxes_tensor[:, 3] - boxes_tensor[:, 1]  
  
        # 重新堆叠，结果形状：[N, 4]，每行是 [cx, cy, w, h]        boxes_converted = torch.stack([cx, cy, w, h], dim=1)  
        ...
        return boxes_normalized # 返回归一化后的边界框张量 [N, 4]    def __len__(self) -> int:  
    ...
    def __getitem__(self, index: int) -> tuple:  
        ...
        # 4. 归一化边界框，归一化后的 (cx, cy, w, h)格式张量，形状 [N, 4]        boxes = self._normalize_boxes(ann['boxes'], ann['orig_size'])  
        ...
        targets = {  
            ...
            'orig_size': torch.tensor(ann['orig_size'], dtype=torch.int64), # [2]，原始尺寸 [H, W]            'track_ids': track_ids, # [N]  
        }  
